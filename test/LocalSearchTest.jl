...
@testset "genetic_tests" begin
    ...
    genf1 = geneticImprovement(G, gen1, f1-f0, 0.1, x, linear_model, 10, 0.1)
    genf2 = geneticImprovement(G, genf1, f2-f1, 0.1, x, linear_model, 10, 0.1)
    genf3 = geneticImprovement(G, genf1, f3-f2, 0.1, x, linear_model, 10, 0.1)
    gen1_losses = pmap(part->getLoss(G, part, x, linear_model, 10, 0.1), gen1)
    gen1_avg = sum(gen1_losses/c)
    genf0_losses = pmap(part->getLoss(G, part, x, linear_model, 10, 0.1), genf0)
    genf0_avg = sum(genf0_losses/c)
    genf1_losses = pmap(part->getLoss(G, part, x, linear_model, 10, 0.1), genf1)
    genf1_avg = sum(genf1_losses/c)
    genf2_losses = pmap(part->getLoss(G, part, x, linear_model, 10, 0.1), genf2)
    genf2_avg = sum(genf2_losses/c)
    genf3_losses = pmap(part->getLoss(G, part, x, linear_model, 10, 0.1), genf3)
