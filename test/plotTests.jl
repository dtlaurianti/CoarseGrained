...
for numOriginalNodes in 100:100:2000
...
for numReducedNodes in 100:100:2000
    push!(x, numReducedNodes)
    Part = generateRandomPartitions(2100, numReducedNodes, 10);
    #LG = line_graph(numOriginalNodes)
    GNP = gnp_graph(2100;p=0.5)
    #CM = cm_graph(numOriginalNodes, [1, 1, 1, 1, 2, 2, 2, 3, 3, 4])
    for i in 1:4
        push!(times, @elapsed(GetXYZ(Part, GNP, 2100, modelType=linear_model)))
    end
    push!(y, median(times))
end
...
for numPartitions in 100:100:2000
